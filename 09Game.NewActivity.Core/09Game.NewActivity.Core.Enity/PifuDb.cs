...
namespace _09Game.NewActivity.Core.Enity
{
    public class PifuDb
    {
        ...
        private readonly ILogger<PifuDb> _logger;

        public PifuDb(ILogger<PifuDb> logger, IMemoryCache cache, PifuDbContext db, IConfiguration configuration, IHttpClientFactory httpClientFactory)
        {
            _cache = cache;
        ...
        public async Task<int> UserPromise(int user_id, int item_id)
        {
            ...
            var money1Balance = moneyResult != null ? moneyResult.Money2Balance : 0;
            ...
            try
            {
                ...
                code = await _db.ExecuteAsync(sql, new
                {
                    user_id = user_id,
                    item_id = item_id,
                    item_name = pifuName,
            ...
            if (code > 0)
            {
                ...
                try
                {
                    var requestv2 = await _bill.Substract(90460, orderNo, user_id, 3, (int)amount);
                ...
                try
                {
                    _logger.LogInformation(reposurl);
        ...
        public async Task<dynamic> GetUserPromiseLog(int user_id)
        {
            ...
            return await _db.QueryFirstOrDefaultAsync<dynamic>(sql, new { user_id });
        }

        public async Task<(int,int)> UserPromiseExchange(int user_id, int item_id)
        {
            if (!_promisePifuName.ContainsKey(item_id))
                throw new ArgumentException("道具id不合法!");
            string pifuName = _promisePifuName[item_id];
            ...
            var userCount = await GetUserPayCount(user_id);

            var userPromise = await GetUserPromiseLog(user_id);
            if (userPromise == null) throw new ArgumentException("未许愿!");
            if (item_id != userPromise.item_id) throw new ArgumentException("未许愿该皮肤!");
            if ((int)userCount < count) throw new ArgumentException("抽奖次数未达到" + count + "次!");

            var status = 0;
            var request = await cbProjects.Query(user_id, item_id);
            var ucount = request.Item.Count == 0 ? 0 : request.Item[0].Amount;
            if (ucount > 0) status = 1;

            try
            {
                var sql = $"INSERT into user_promise_exchange_log(user_id,item_id,item_name,status,create_at) VALUES(@user_id,@item_id,@item_name,@status,NOW())";
            ...
            {
                throw new ArgumentException("已被领取过!");
            }
        ...
        /// <param name="user_id"></param>
        /// <returns></returns>
        public async Task<int> GetUserPayCount(int user_id)
        {
            string sql = "SELECT pay_count from user_pifu_lottery where user_id=@user_id";
            return await _db.ExecuteScalarAsync<int>(sql, new { user_id });
        }
        ...
        public async Task<dynamic> GetUserEnergyLog(int user_id,int type)
        {
            ...
            return await _db.QueryFirstOrDefaultAsync<dynamic>(sql, new { user_id, type });
        }
        ...
        public async Task<int> TestAdd(int user_id)
        {
            var orderNo = Guid.NewGuid().ToString();

            var requestv2 = await cbProjects.Subtract(user_id, 90440, orderNo, 20092, 10);
            var bill_order = requestv2.Order;

            var requestv3 = await cbProjects.Rollback(user_id, 90440, orderNo, bill_order, "测试回滚");

            //var requestv2 = await _bill.Substract(90460, orderNo, user_id, 3, 1);
            //if (requestv2 == null || requestv2.Result != 0)
            //{
            //    Logger.Error($"宝箱许愿扣钱:user_id ={user_id},Result={requestv2.Result}");
            //    throw new ArgumentException("扣钱失败!");
            //}

            //var bill_order = requestv2.BillOrder;

            //var requestv3 = await _bill.Rollback(user_id, 90440, bill_order, orderNo);
